...
//        $featuredData = collect($paginateFeatured->items())->buildContentResourceData();
    ...
    public function homepage()
    {
        ...
        # 直接找出首頁要顯示的文章id
        $item_ids = ExtrafieldValue::whereIn('extrafield_id', [10, 19])->where('value', 1)->get()->pluck('item_id');
...
//            $itemSearchData['limit'] = 200;
    ...
    protected function searchFeatureContent(Collection $input, $content_type)
    {
        ...
        $notFeatured = $notFeatured->merge($featured_redundant->filter(function ($r) use ($input) {
            ...
            return count($brand_alias) && $r->brands()->first()->alias == $input->get('brand_alias')[0];
