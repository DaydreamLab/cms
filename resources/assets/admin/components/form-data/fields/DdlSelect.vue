<template>
    <div>
        <el-select
                ...
                :placeholder="$t('OPTION_SELECT', { name: data.desc })"
                :clearable='data.clearable ? true : false'>
            <el-option
      ...
      onChange(v) {
        ...
        if (v) {
          if (Array.isArray(v)) {
            ...
            v.forEach(item => {
              this.submit_info[this.data.key].push(
                this.temp_field_obj[this.data.key][item]
              );
            });
          } else {
            this.submit_info[this.data.key] = "";
            this.submit_info[this.data.key] = this.temp_field_obj[
              this.data.key
            ][v];
          }
        }
