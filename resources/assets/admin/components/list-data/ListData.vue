<template>
    <div class="list-data">
        <ddl-table-header
                ...
                :Search="searchbar">
            <span slot="ddl-header-after"><slot name="header-after"></slot></span>
            <span slot="ddl-header-before"><h2>{{ $t($route.matched[$route.matched.length - 1].name) }}</h2><slot name="header-before"></slot></span>
        </ddl-table-header>
        ...
        <el-table border style="width: 100%" align='center'
            ...
            <el-table-column v-if='sort && sort.show'
                             width="45"
                             prop="ordering">
                            <template slot-scope="scope">
                ...
                <el-table-column
                    ...
                    <template slot-scope='scope'>
                        <div class="text-label" :class="scope.column.formatter(scope.row[field.key])['color']">
                            {{ scope.column.formatter(scope.row[field.key])['text'] }}
                        </div>
                    </template>
                </el-table-column>
                <el-table-column
                ...
                <el-table-column
                    ...
                    <template slot-scope='scope'>
                        <el-tooltip placement="top">
                            <div slot="content">
                                <strong>{{ $t('TOOLBAR_CHECKOUT')/*回存*/ }}</strong><br />
                                {{ scope.row['locker'] }}<br />
                                {{ scope.row['locked_at'] }}
                            </div>
                            <el-button class="btn--checkout" size="mini" v-if="scope.row['locked_by']" @click='onBtnEvent({type:"checkout",data:scope.row,dataIndex:scope.$index,list:list})'>
                                <font-awesome-icon :icon="['fal', 'lock-alt']"></font-awesome-icon>
                            </el-button>
                        </el-tooltip>
                        <span class="editable_title" v-can="'edit'" @click='onBtnEvent({type:"edit",data:scope.row,dataIndex:scope.$index,list:list})' v-html="(scope.column.formatter && scope.column.formatter(scope.row[field.key])) || scope.row[field.key]" ></span>
                        <span v-can:not="'edit'" v-html="scope.row[field.key]"></span>
                    </template>
                </el-table-column>
            </template>
            <el-table-column
                v-if="list_actions.show"
